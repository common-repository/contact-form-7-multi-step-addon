...
class Multistep_Form {
    ...
    public function add_tag_generator_menu() {
        ...
        $tag_generator->add( 'progressbar', esc_html__('Progressbar', 'contact-form-multi-step-addon' ), array($this, 'progressbar_shortcode_generator'));

        $tag_generator->add( 'step_start', esc_html__('Step start', 'contact-form-multi-step-addon' ), array($this, 'step_shortcode_generator'));
        $tag_generator->add( 'step_finish', esc_html__('Step finish', 'contact-form-multi-step-addon' ), array($this, 'step_shortcode_generator'));

        $tag_generator->add( 'next', esc_html__('Next', 'contact-form-multi-step-addon' ), array($this, 'next_shortcode_generator'));

        $tag_generator->add( 'prev', esc_html__('Prev', 'contact-form-multi-step-addon' ), array($this, 'prev_shortcode_generator'));
    }
    ...
    public function progressbar_shortcode_generator($contact_form, $args = '') {
        ...
        set_query_var( 'trx_shortcode_args', array(
            'attrs' => array(
                'type' => array(
                    'label' => esc_html__('Progressbar type', 'contact-form-multi-step-addon'),
                    'type' => 'select',
                    'default' => 'bullets',
                    'options' => array(
                        'numbers' => esc_html__('Numbers', 'contact-form-multi-step-addon'),
                        'bullets' => esc_html__('Bullets', 'contact-form-multi-step-addon'),
                        'line' => esc_html__('Just Line', 'contact-form-multi-step-addon'),
                    )
                )
            ),
            'args' => $args
    ...
    public function step_shortcode_generator($contact_form, $args = '') {
        ...
        if ($args['id'] === 'step_start') {
            $attrs['values'] = array(
                'label' => esc_html__('Label', 'contact-form-multi-step-addon'),
                'type' => 'text',
                'default' => esc_html__('Step 1', 'contact-form-multi-step-addon')
            );
        }
        ...
        set_query_var( 'trx_shortcode_args', array(
            'attrs' => array(
                'values' => array(
                    'label' => esc_html__('Label', 'contact-form-multi-step-addon'),
                    'type' => 'text',
                    'default' => esc_html__('Next', 'contact-form-multi-step-addon')
                )
            ),
            'args' => $args
        ) );
        ...
        set_query_var( 'trx_shortcode_args', array(
            'attrs' => array(
                'values' => array(
                    'label' => esc_html__('Label', 'contact-form-multi-step-addon'),
                    'type' => 'text',
                    'default' => esc_html__('Previous', 'contact-form-multi-step-addon')
                )
            ),
            'args' => $args
        ) );
    ...
    public function validate_fields_ajax() {
        if ( !wp_verify_nonce($_REQUEST['trx_ajax_nonce'], TRX_MSCF_PLUGIN_FILE)) {
            exit(esc_html__("Security error", 'contact-form-multi-step-addon'));
        }
